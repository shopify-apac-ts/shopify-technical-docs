...
### What happens to retailer data and order amounts if the supplier changes product data, price, or inventory?

Inventory and cost price are continuously synced from the supplier to retailer stores. Retail price sync is controlled separately: retailers can manage retail price themselves, or they can enable retail price syncing from the supplier. Other product details, such as title, description, media, and category, can be copied and optionally kept in sync depending on product policy and sync settings. For future orders, supplier-side cost price, supplier shipping, retail price sync settings, and inventory changes can affect retailer economics and the supplier order amount. Existing paid orders keep the amounts captured at order time unless they are changed through order edits, cancellations, refunds, returns, or payment adjustments. Retailer-side integrations that need to mirror these changes to an external system should listen to product creation, update, and deletion webhooks.

### What happens to supplier data and order amounts if the retailer changes product data, price, or inventory?

Retailer-side edits do not update the supplier's source product. Retailers can edit certain buyer-facing product details such as retail price, title, description, and media, but supplier-managed fields remain controlled by the supplier. Inventory quantities for imported Collective products are managed by the supplier and are read-only in the retailer admin. Retailer edits to synced or supplier-controlled fields can be overwritten when the supplier updates the original product or can break expected sync behavior if the Collective inventory location or product connection is changed. Retailer-side price edits, discounts, and content edits do not change the supplier cost price or supplier master data. The supplier order amount is still based on supplier cost price, supplier shipping, applicable taxes, and order or payment adjustments. External systems connected to the retailer store should treat product webhooks as retailer-store updates, not as evidence that supplier master data changed.

### What happens to the supplier order amount if the retailer configures a discount or sells below the supplier-side amount?

Retailer discounts and buyer-facing price changes affect what the customer pays and the retailer's profit, but they do not change the supplier cost price for the item. If the retailer discounts the item, or sets a buyer-facing price below the supplier-side cost amount, the retailer can create a lower or negative margin while still owing the supplier based on the supplier cost price. For example, if a product has a retail price of 100 USD and a supplier cost price of 70 USD, a 20 USD customer discount reduces the retailer's profit but the retailer still owes the supplier 70 USD for the product. Shipping, taxes, refunds, returns, and payment adjustments still need to be reconciled separately.
...
### What changes in order data and operations when an order is paid with Shopify Payments versus another payment method?

The customer payment gateway does not change the core order data flow by itself. The retailer order is still created first, and after payment and fraud checks, Collective creates or links the supplier order and fulfillment request. External integrations should continue to use order and fulfillment webhooks as the operational change feed. The important operational difference is whether the Collective supplier payment is handled through Automatic Payments. When Automatic Payments are enabled and the order is eligible for automated payment handling, Collective records Shopify Payments balance operations: the retailer balance is debited for supplier product, shipping, taxes, and adjustments, and the supplier balance is credited after fulfillment. If Automatic Payments are off or the order is not handled through automated payments, the order and fulfillment records can still be created, but Shopify does not create the automatic retailer debit and supplier credit balance operations for settlement. Supplier payment then remains a manual or pending reconciliation item, and the merchants or external finance system need to reconcile payment outside Shopify Payments balance adjustments.
